    ...
    <MainLayout>
      ...
      >
        ...
        >
          ...
          <div class="text-white flex gap-3 w-2/12">
            <VaSelect
                v-model="searchYears"
                :options="years"
                ...
                searchable
                @update:modelValue="searchChange('searchYears')"
                class="w-1/2"
            />
            <VaSelect
                ...
                placeholder="Année scolaire"
                searchable
                @update:modelValue="searchChange('searchClasses')"
                class="w-1/2"
            />
          </div>
        ...
        <table class="w-full text-sm text-left rtl:text-white text-white">
          ...
          <tbody v-if="all.loading || !all.data.length">
            <!-- Skeleton -->
            <tr class="border-b bg-gray-700 hover:bg-gray-100 h-12" v-if="all.loading" v-for="i of [1,4,5,6,7,8]" :key="i">
...
import Modal from "@/views/students/Modal.vue";
...
const { all } = storeToRefs($annexesStore);
const annexesTitle = ref("Elèves");
...
const years = ref([]);
const searchYears = ref({ text: "Année scolaire", value: null});
const classes = ref([]);
const searchClasses = ref({ text: "Classe", value: null});
...
const searchChange = async (type)=> {
  if (type === "searchYears") {
    await $classesStore.getAll({yearsId: searchYears.value.value});
    classes.value = $classesStore.all.data.map((classe) => {
      return {text: classe.label, value: classe.id};
    });
    searchClasses.value = classes.value[0] ? classes.value[0] : { text: "Classe", value: null };
  }
  await fetchPaginatedAnnexes();
};
const  toggleAll = () => {
  if (selectedAnnexes.value.length === $annexesStore.all.data.length) {
    selectedAnnexes.value = [];
    $annexesStore.all.data.forEach((annexe) => {
...
const toggleSelection = (annexe) => {
  ...
  if (annexe.selected) {
    ...
    selectedAnnexes.value = selectedAnnexes.value.filter((n) => n.id !== annexe.id);
  }
};
...
const fetchPaginatedAnnexes = async () => {
  ...
  params = {
    label: search.value,
    yearsId: searchYears.value.text === "Année scolaire" ? null : searchYears.value.value,
    classesId: searchClasses.value.text === "Classe" ? null : searchClasses.value.value,
...
onMounted(async () => {
  ...
  years.value = $yearsStore.years.data.map((year) => {
    return {text: year.label, value: year.id};
  });
  searchYears.value = years.value[0] ? years.value[0] : { text: "Année scolaire", value: null };
  const params = {
    label: search.value,
    yearsId: searchYears.value.text === "Année scolaire" ? null : searchYears.value.value,
    sort_field: sortField.value,
