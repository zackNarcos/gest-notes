            ...
            >
              ...
              >
                ...
                >
                  {{
                    annexe.id
                      ? `Mettre à jour un Eleve: ${annexe.first_name}`
              ...
              <form @submit.prevent="onSubmit">
                <div class="bg-gray-700 text-white px-4 pt-5 pb-4">
                  <div class="flex flex-direction-column">
                    <span class="mb-2"> Nom<span class="text-red-500  text-[18px]"> *</span></span>
                    <VaInput v-model="annexe.firstName"
                              placeholder="Saisir un nom"
                    />
                  </div>
                  <div class="flex flex-direction-column">
                    <span class="mb-2"> Prénom<span class="text-red-500  text-[18px]"> *</span></span>
                    <VaInput v-model="annexe.lastName"
                              placeholder="Saisir un prénom"
                    />
                  </div>
                  <div class="flex flex-direction-column">
                    <span class="mb-2"> E-mail<span class="text-red-500  text-[18px]"> *</span></span>
                    <VaInput v-model="annexe.email"
                              placeholder="Saisir un mail"
                    />
                  </div>
                  <div class="flex flex-direction-column">
                    <span class="mb-2"> Téléphone<span class="text-red-500  text-[18px]"> *</span></span>
                    <VaInput v-model="annexe.tel"
                  ...
                  <div class="flex flex-direction-column mt-2 z-20">
                    ...
                    <VaSelect v-model="selecteYear"
                              ...
                              searchable
                              @update:modelValue="fetchClasses"
                    />
                  </div>
                  <div class="flex flex-direction-column mt-2 z-20">
                    <span class="mb-2">Classe<span class="text-red-500  text-[18px]"> *</span></span>
                    <VaSelect v-model="selecteClasse"
                              :options="allAllClasses"
                              :text-by="option => option.text"
                              placeholder="Sélectionner une année"
                              searchable
                    />
                ...
                >
                  <button
                    :disabled="!annexe.firstName || !annexe.lastName || !annexe.email || !selecteYear.value || !selecteClasse.value"
                    type="submit"
                    :class="(!annexe.label || !annexe.lastName || !annexe.email || !selecteYear.value || !selecteClasse.value ) ? 'bg-pink-700/50 hover:bg-pink-800/50' : 'bg-pink-600 hover:bg-pink-700'"
...
const $classesStore = useClasseStore();
const $annexesStore = useStudentStore();
...
onUpdated(async () => {
  ...
  $classesStore.all.data.forEach((classe) => {
    if (classe.id === props.annexe.classesId) {
      selecteClasse.value = {text: classe.label, value: classe.id};
    }
  });
  annexe.value = {
    id: props.annexe.id,
    firstName: props.annexe.firstName,
    lastName: props.annexe.lastName,
    email: props.annexe.email,
    tel: props.annexe.tel,
    year: selecteYear.value,
    classe: selecteClasse.value,
  };
...
const onSubmit = async () => {
  ...
  let tmpAnnexe = {
    id: annexe.value.id,
    firstName: annexe.value.firstName,
    lastName: annexe.value.lastName,
    email: annexe.value.email,
    tel: annexe.value.tel,
    yearsId: selecteYear.value.value,
    classesId: selecteClasse.value.value,
  };
...
const fetchClasses = async () => {
  ...
  await $classesStore.getAll(params);
  $classesStore.all.data.forEach((classe) => {
    if (classe.id === props.annexe.classesId) {
      selecteClasse.value = {text: classe.label, value: classe.id};
    }
  });
  allAllClasses.value = $classesStore.all.data.map((classe) => {
    return {text: classe.label, value: classe.id};
  });
  selecteClasse.value = allAllClasses.value[0];
};
onMounted(async () => {
  await fetchYears()
  await fetchClasses()
